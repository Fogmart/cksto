<?php
class ModelToolProfiImport extends Model {
  ...
  public function getXmlTags($import_id){
    $return = array();
    $query = $this->db->query("SELECT distinct(tag_name),tag_key,level FROM `" . DB_PREFIX . "profi_xml_import_tags_cache` WHERE import_id = '".(int)$import_id."' ORDER by tag_cache_id ASC");
    if($query->rows){
      $main_tag_name = '';
      $prev_tag_name = '';
  ...
  public function checkDatabasePrepare(){
    $result = false;
    $table_exists = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "profi_xml_import'");
    if(!$table_exists->row){
      $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "profi_xml_import` (`import_id` int(11) NOT NULL AUTO_INCREMENT,`name` varchar(255) CHARACTER SET utf8 NOT NULL,`xml_url` text CHARACTER SET utf8 NOT NULL,`import_info` text CHARACTER SET utf8 NOT NULL,`parts` int(2) NOT NULL DEFAULT '1',`download_image` tinyint(1) NOT NULL DEFAULT '1',`product_tag` varchar(255) CHARACTER SET utf8 NOT NULL,`primary_key` varchar(255) CHARACTER SET utf8 NOT NULL,`stock_status_id` int(11) NOT NULL,`tax_class_id` int(11) NOT NULL,`length_class_id` int(11) NOT NULL,`weight_class_id` int(11) NOT NULL,`manufacturer_id` int(11) NOT NULL,`attribute_group_name` varchar(255) CHARACTER SET utf8 NOT NULL,`option_group_name` varchar(255) CHARACTER SET utf8 NOT NULL,`option_type` varchar(255) CHARACTER SET utf8 NOT NULL,`option_quantity` int(4) NOT NULL,`option_subtract` tinyint(1) NOT NULL DEFAULT '0',`option_required` tinyint(1) NOT NULL DEFAULT '1',`global_language_id` int(11) NOT NULL,`category_separator` varchar(255) CHARACTER SET utf8 NOT NULL,`seo_keyword_product` varchar(255) CHARACTER SET utf8 NOT NULL,`seo_keyword_category` varchar(255) CHARACTER SET utf8 NOT NULL,`seo_keyword_manufacturer` varchar(255) CHARACTER SET utf8 NOT NULL,`old_product_action` varchar(255) CHARACTER SET utf8 NOT NULL,`product_status` tinyint(1) NOT NULL DEFAULT '1',`product_subtract` tinyint(1) NOT NULL DEFAULT '1',`product_quantity` int(4) NOT NULL DEFAULT '0',`price_edit` decimal(10,4) NOT NULL,`price_edit_type` varchar(255) CHARACTER SET utf8 NOT NULL,`price_edit_options` tinyint(1) NOT NULL DEFAULT '0',`store_id` text CHARACTER SET utf8 NOT NULL,`update_items` text CHARACTER SET utf8 NOT NULL,`date_last_import` datetime NOT NULL,`date_added` datetime NOT NULL,`date_changed` datetime NOT NULL,PRIMARY KEY (`import_id`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;");
      $this->db->query("ALTER TABLE `" . DB_PREFIX . "profi_xml_import` ADD `special_price_customer_group_id` INT(11) NOT NULL DEFAULT '1' AFTER `product_quantity`;");
      $this->db->query("ALTER TABLE `" . DB_PREFIX . "profi_xml_import` ADD `product_shipping` TINYINT(1) NOT NULL DEFAULT '1' AFTER `product_subtract`; ");
    ...
    $table_exists = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "profi_xml_import_tags'");
    if(!$table_exists->row){
      $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "profi_xml_import_tags` (`tag_id` int(11) NOT NULL AUTO_INCREMENT,`import_id` int(11) NOT NULL,`tag_name` varchar(255) CHARACTER SET utf8 NOT NULL,`tag_content` varchar(255) CHARACTER SET utf8 NOT NULL,PRIMARY KEY (`tag_id`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;");
    }
    $table_exists = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "profi_xml_import_tags_cache'");
    if(!$table_exists->row){
      $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "profi_xml_import_tags_cache` (`tag_cache_id` int(11) NOT NULL AUTO_INCREMENT,`import_id` int(11) NOT NULL,`tag_name` varchar(255) CHARACTER SET utf8 NOT NULL,`tag_content` text CHARACTER SET utf8 NOT NULL,`tag_key` varchar(255) CHARACTER SET utf8 NOT NULL,`tag_key_content` varchar(255) CHARACTER SET utf8 NOT NULL,`level` int(10) NOT NULL,PRIMARY KEY (`tag_cache_id`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;");
    }
    return $result;
  }
    ...
    $this->db->query("UPDATE " . DB_PREFIX . "profi_xml_import SET import_info = '".serialize($import_info)."' WHERE import_id = '".(int)$import_id."'");
  }
...
//fix for last tags inline - file damaged:
  ...
  //import info - update
    ...
    $this->db->query("DELETE FROM " . DB_PREFIX . "profi_xml_import WHERE import_id = '".(int)$import_id."'");
    $this->db->query("DELETE FROM " . DB_PREFIX . "profi_xml_import_tags WHERE import_id = '".(int)$import_id."'");
    $this->db->query("DELETE FROM " . DB_PREFIX . "profi_xml_import_tags_cache WHERE import_id = '".(int)$import_id."'");
    if($delete_products){
	  	$this->load->model('catalog/product');
	...
	public function getImports(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profi_xml_import ORDER by name ASC");
    if(!$query->rows){
      return false;
    }else{
      return $query->rows;
	...
	public function getImportTags($import_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profi_xml_import_tags WHERE import_id = '".(int)$import_id."'");
    if(!$query->rows){
      return array();
    }else{
      return $query->rows;
	...
	public function getImport($import_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profi_xml_import WHERE import_id = '".(int)$import_id."'");
    if(!$query->row){
      return false;
    }else{
	...
	public function addImport($import_name){
		$this->db->query("INSERT INTO " . DB_PREFIX . "profi_xml_import SET name = '".$this->db->escape($import_name)."', date_added = NOW()");
	...
	public function updateImport($import_id, $feed_data){
		$this->db->query("UPDATE " . DB_PREFIX . "profi_xml_import SET 
...
product_shipping = '".(int)$feed_data['product_shipping']."'
 ...
 WHERE import_id = '".(int)$import_id."'");
    ...
    foreach($feed_data['tags'] as $tag_name => $tag_content){
      if($tag_content != "-"){
	     	$this->db->query("INSERT INTO " . DB_PREFIX . "profi_xml_import_tags SET import_id = '".(int)$import_id."', tag_name = '".$this->db->escape($tag_name)."', tag_content = '".$this->db->escape($tag_content)."'");
      }
    }
  ...
  public function getTagsCache($import_id){
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profi_xml_import_tags_cache WHERE import_id = '".(int)$import_id."' ORDER by tag_cache_id ASC");
    return $query->rows;
  }  
	...
	public function getLastImportId(){
		$query = $this->db->query("SELECT import_id FROM " . DB_PREFIX . "profi_xml_import ORDER by import_id DESC LIMIT 1");
	...
	public function getTagAssignContent($import_id,$tag_name){
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profi_xml_import_tags WHERE import_id = '".(int)$import_id."' AND tag_name = '".$this->db->escape($tag_name)."'");
	  if($query->row){
  ...
  if(isset($product_data['categories'])){
    foreach($product_data['categories'] as $index => $category){
      ...
      foreach($this->getLanguages() as $language){
        ...
        if(!isset($category[$language_id])){$category_detail[$language_id] = $category[$import_data['global_language_id']];}
        else{$category_detail[$language_id] = $category[$language_id];}
      }
...
*/
        ...
        if(!isset($product_data['product_descriptions'][$language_id]['name'])){
          if(isset($product_data['product_descriptions'][$import_data['global_language_id']]['name'])){$product_data['product_descriptions'][$language_id]['name'] = $product_data['product_descriptions'][$import_data['global_language_id']]['name'];}
          else{$product_data['product_descriptions'][$language_id]['name'] = false;}
        }
        if(!isset($product_data['product_descriptions'][$language_id]['description'])){
          if(isset($product_data['product_descriptions'][$import_data['global_language_id']]['description'])){$product_data['product_descriptions'][$language_id]['description'] = $product_data['product_descriptions'][$import_data['global_language_id']]['description'];}
          else{$product_data['product_descriptions'][$language_id]['description'] = false;}
        }
      ...
      foreach($this->getLanguages() as $language){
        ...
        if(!isset($product_data['product_attributes'][$i][$language_id])){
          ...
          $product_data['product_attributes'][$i][$language_id] = array(
            'group' => $group_name,
            'name'  => $product_data['product_attributes'][$i][$import_data['global_language_id']]['name'],
            'text'  => $product_data['product_attributes'][$i][$import_data['global_language_id']]['text'],
          );
        }else{
...
public function getOrInsertCategory($category,$import_data){
...
//set categories for another descriptions:
...
public function insertUrlAlias($url_query,$url_keyword){
  $url_keyword = $this->createSEOKeyword($url_keyword);
