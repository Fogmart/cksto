...
class ControllerExtensionModuleSodealscategory extends Controller {
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'], 'SSL'));
		}
	// Get Module Id new 
		$moduleid_new= $this->model_extension_module_so_deals_category->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
				$module_id = $moduleid_new[0]['Auto_increment'];
				$this->model_extension_module->addModule('so_deals_category', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_deals_category', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

		$moduleid_new= $this->model_extension_module_so_deals_category->getModuleId(); // Get module id
		...
		$data['moduletabs'] = $this->model_extension_module->getModulesByCode( 'so_deals_category' );
		$data['link'] = $this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/module/so_deals_category&token=' . $this->session->data['token']);
		//--------------------------------Load Data -------------------------------------------
		...
		//=== Theme Custom Code====
		$data['type_layout'] = $this->getLayoutMod('so_deals_category');
		...
		$this->response->setOutput($this->load->view('extension/module/so_deals_category.tpl', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_deals_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/so_deals_category')) {
	...
	public function autocomplete_category() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_module_so_deals_category->getCategories($filter_data);
