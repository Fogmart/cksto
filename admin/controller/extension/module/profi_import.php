<?php
class ControllerExtensionModuleProfiImport extends Controller {
	private $error      = array(); 
	private $tag_number = 0; 

	public function index() {
		$this->language->load('extension/module/profi_import');
  ...
  //texts:
    ...
    $data['update_items'] = array(
      ...
      'status' => "Product status",
		...
		$this->load->model('tool/profi_import');
    $this->model_tool_profi_import->checkDatabasePrepare();
    ...
    $data['imports']   = $this->model_tool_profi_import->getImports();
    
    
    $data['stock_statuses']   = $this->model_tool_profi_import->getStockStatuses();
    $data['tax_classes']      = $this->model_tool_profi_import->getTaxClasses();
    $data['length_classes']   = $this->model_tool_profi_import->getLengthClasses();
    $data['weight_classes']   = $this->model_tool_profi_import->getWeightClasses();
    $data['manufacturers']    = $this->model_tool_profi_import->getManufacturers();
    $data['stores']           = $this->model_tool_profi_import->getStores();
    $data['languages']        = $this->model_tool_profi_import->getLanguages();
    $data['customer_groups']  = $this->model_tool_profi_import->getCustomerGroups();


    if(isset($_POST['add_import'])){
      $import_name = htmlspecialchars($_POST['import_name']);
      $import_id = $this->model_tool_profi_import->addImport($import_name);
			$this->session->data['success'] = $this->language->get('text_insert_import_success');
			$this->response->redirect($this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'] . '&import_id='.(int)$import_id, true));
    }
    ...
    if(isset($_GET['action']) AND $_GET['action'] == "deleteImport"){
      ...
      $this->model_tool_profi_import->deleteImport($import_id,$include_products);
			$this->session->data['success'] = $this->language->get('text_delete_success');
			$this->response->redirect($this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'], true));
    }
      ...
      $feed_data = array(
        ...
        'product_tag'              => $_POST['feed_data']['product_tag'],
      ...
      $this->model_tool_profi_import->updateImport($import_id,$feed_data);
      $this_import = $this->model_tool_profi_import->getImport($import_id);
			$this->session->data['success'] = sprintf($this->language->get('text_success_update'),$this_import['name']);
			$this->response->redirect($this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'].'&import_id='.$import_id, true));
		}
    
    
//links
    $data['download_xml_url']                    = $this->url->link('extension/module/profi_import/download_xml', 'import_id='.$import_id.'&token=' . $this->session->data['token'], true);
    $data['xml_structure_url']                   = $this->url->link('extension/module/profi_import/getXMLStructure', 'import_id='.$import_id.'&token=' . $this->session->data['token'], true);
    $data['link_redirect_link']                  = $this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'], true);
    $data['link_delete_import']                  = $this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'] . '&action=deleteImport&include_products=false&import_id='.$import_id, true);
    $data['link_delete_import_include_products'] = $this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'] . '&action=deleteImport&include_products=true&import_id='.$import_id, true);
    $data['link_product_tags']                   = $this->url->link('extension/module/profi_import/getProductTagsOptions', 'token=' . $this->session->data['token'], true);
    $data['link_import_info']                    = $this->url->link('extension/module/profi_import/getXMLInfo', 'import_id='.$import_id.'&token=' . $this->session->data['token'], true);
    ...
    if($import_id){
      ...
      $data['feed_data'] = $this->model_tool_profi_import->getImport($import_id);
      ...
      $xml_structure = $this->model_tool_profi_import->getTagsCache($import_id);
      ...
      if($xml_structure){
        ...
        $data['xml_tags']      = $this->model_tool_profi_import->getXmlTags($import_id);
      ...
      $data['tag_options'] = $this->getTagOptions();
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/profi_import', 'token=' . $this->session->data['token'], true),
			'separator' => ' :: '
		);
		...
		$this->response->setOutput($this->load->view('extension/module/profi_import', $data));
	}
    ...
    if($import_id){
      ...
      $xml_structure = $this->model_tool_profi_import->getXMLStructure($xml_file);
      $this_import   = $this->model_tool_profi_import->getImport($import_id);
      $html          = '';
      if($xml_structure){
        foreach($xml_structure as $tag){
    ...
    $import_id = (int)$_GET['import_id'];
		$this->language->load('extension/module/profi_import');
 		$this->load->model('tool/profi_import');
    $import_data = $this->model_tool_profi_import->getImport($import_id);
  ...
  public function download_xml() {
    ...
    if(isset($_POST['download_xml'])){
      ...
      $import_id = htmlspecialchars($_POST['import_id']);
   		$this->load->model('tool/profi_import');
      $download = $this->model_tool_profi_import->downloadXML($import_id,$xml_url);
	...
	public function getTagOptions(){
		$this->language->load('extension/module/profi_import');
		$this->load->model('tool/profi_import');
    $languages                 = $this->model_tool_profi_import->getLanguages();
    $tag_options               = array();
        ...
        public function getTagRow($import_id,$level,$tag_name,$tag_value,$tag_options,$tag_path = array()){
          ...
          if(isset($tag_options)){
            foreach($tag_options as $option_key => $option_title){
              ...
              $selected = '';
              $have_assign_content = $this->model_tool_profi_import->getTagAssignContent($import_id,implode(';',$tag_path));
