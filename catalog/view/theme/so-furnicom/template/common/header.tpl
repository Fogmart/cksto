...
<?php // Begin General CSS ----- 
	...
	if($direction=='rtl') $this->soconfig->addCss('catalog/view/javascript/soconfig/css/bootstrap/bootstrap.rtl.min.css');
...
<?php // Begin Themes Scripts ----
	...
	$this->soconfig->addJs('catalog/view/javascript/soconfig/js/cpanel.js');
...
<?php foreach ($analytics as $analytics) { ?>
<?php echo $analytics; ?>
<?php } ?>
...
<div id="wrapper" class="<?php echo $cls_wrapper; ?>">   
	...
	if(isset($typeheader)){
		switch ($typeheader) {
		case "1":
			include(DIR_TEMPLATE.$theme.'/template/header/header1.tpl');break;
		case "2":
			include(DIR_TEMPLATE.$theme.'/template/header/header2.tpl');break;
		}
	}else{
