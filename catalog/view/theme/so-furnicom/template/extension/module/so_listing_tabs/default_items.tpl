<?php
 if ($type_show == 'slider') { ?>
		<div class="ltabs-items-inner products-list grid owl2-carousel  ltabs-slider ">
<?php }else { ?>
		<div class="ltabs-items-inner  products-list grid <?php echo  $type_show == 'loadmore' ? $class_ltabs. ' '. $effect : '  ' ;?>">
<?php } ?>
<?php 
	if (!empty($child_items)) {
		...
		foreach ($child_items as $product) {
			...
			<?php } ?>
				...
				<div class="item-inner product-layout  product-thumb">
					...
					<?php } if($display_add_to_cart || $display_wishlist || $display_compare){?>
						...
						<?php if($display_add_to_cart){?>
							<button class="addToCart btn-button" type="button" data-toggle="tooltip" title="<?php echo $objlang->get('button_cart'); ?>" onclick="cart.add('<?php echo $product['product_id']; ?>');"><span><?php echo $objlang->get('button_cart'); ?></span></button>
