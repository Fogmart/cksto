...
<button class="addToCart btn-button" type="button" data-toggle="tooltip" title=""
        onclick="chkPhone()"
        data-original-title="<?php echo $button_confirmation?>">
</button>
...
<input class="addToCart btn-button" type="button" value="Ok" data-toggle="tooltip" title="" onclick="chkCode()" data-original-title="OK" style="margin-top:5px;line-height: 5px;">
<?php $sms = $_POST['result']; echo $result;?>
        ...
        <div class="form-group required" style="display: <?php echo (count($customer_groups) > 1 ? 'block' : 'none'); ?>;">
            ...
            <div class="col-sm-10" style="column-count:2">
          ...
          <div class="form-group inn" style="display: none">
              <label class="col-sm-2 control-label" for="input-inn"><?php echo $entry_inn; ?></label>
              <div class="col-sm-10">
                  <input type="text" name="inn" value="<?php echo $inn; ?>" placeholder="<?php echo $entry_inn; ?>" id="input-inn" class="form-control" />
                  <?php if ($error_inn) { ?>
...
function chkCode() {
    // someFunc()
    $("#codemsg").empty()
    $.post("index.php?route=account/register/cnfrmcode",
            {code: document.getElementById("quantity").value}, function (data) {
            $("#codemsg").html(data)
        }
    );
}
...
function authorization(min,max)
{
	...
	//var sendsms ="http://api.infosmska.ru/interfaces/SendMessages.ashx?login=sks-avto&pwd=vfhfpv19725&phones=" + tel + "&message="+ sms + "&sender=Avtomarket";
	//var sendsms ="http://api.infosmska.ru/interfaces/SendMessages.ashx?login=glinda&pwd=vfhfpv19725&phones=" + tel + "&message="+ sms + "&sender=Informator";
	var sendsms ="https://smsc.ru/sys/send.php?login=glinda&psw=vfhfpv1972&phones=" + tel +"&mes="+ sms;
	//alert (sendsms);
	var xhr = new XMLHttpRequest();
	xhr.open("GET",sendsms, true);
	xhr.send();	
...
$(function () {
    $("[name=customer_group_id]").click(updInnViz)
    updInnViz();
})

function updInnViz() {
    var v = $("[name=customer_group_id]:checked").val()
    if (v == "5" || v == "3" || v == "4"  ){
        $(".inn").slideDown()
    } else {
        $(".inn").slideUp()
