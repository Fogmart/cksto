	...
	class ControllerCommonSeoBlog extends Controller {
		...
		public function __construct($registry) {
			...
			if (!$this->cache_data) {
				...
				if (isset($this->data['settings_general']['latest_widget_status']) && $this->data['settings_general']['latest_widget_status']) {
					$query = $this->db->query("SELECT blog_id, design, path FROM " . DB_PREFIX . "blog");
					foreach ($query->rows as $row) {
						if (isset($row['design']) && $row['design'] != '') {
							$this->cache_data['blog_design'][$row['blog_id']] = unserialize($row['design']);
						} else {
							$this->cache_data['blog_design'][$row['blog_id']] = Array();
						}
						if (isset($row['path']) && $row['path'] != '') {
							$this->cache_data['blog_path'][$row['blog_id']] = $row['path'];
						}
					}
				}
		...
		private function getSEOPathByRecord($record_id) {
            ...
            if (isset($this->cache_data['record_id='.$record_id]) && $this->cache_data['record_id='.$record_id]['blog_id'] != '') {
            	$this->blog_id_current = $blog_id = $this->cache_data['record_id='.$record_id]['blog_id'];

				if (isset($this->cache_data['blog_design'][$blog_id]) && !empty($this->cache_data['blog_design'][$blog_id])) {
		            $this->blog_design[$blog_id] = $this->cache_data['blog_design'][$blog_id];
				}
				if (isset($this->blog_design[$blog_id]['blog_short_path']) && $this->blog_design[$blog_id]['blog_short_path'] == 1) {
					return '';
				}
		        if (isset($this->cache_data['blog_path'][$blog_id]) && $this->cache_data['blog_path'][$blog_id] != '') {
					$record_path = $this->cache_data['blog_path'][$blog_id];
			...
			WHERE
			r2b.record_id = '" . (int) $record_id . "'
			ORDER BY blog_main DESC LIMIT 1";
				$query = $this->db->query($sql);
		...
		private function getSEOPathByBlog($blog_id) {
			...
			if (strpos($blog_id, '_') !== false) {
				...
				$blog_id = (int) end($array_blog_id);
			}

			$this->blog_id_current = $blog_id;
            ...
            if (isset($this->cache_data['blog_design'][$blog_id]) && !empty($this->cache_data['blog_design'][$blog_id])) {
	            $this->blog_design[$blog_id] = $this->cache_data['blog_design'][$blog_id];
		        if (isset($this->cache_data['blog_path'][$blog_id]) && $this->cache_data['blog_path'][$blog_id] != '') {
					return $this->cache_data['blog_path'][$blog_id];
				}
			}
