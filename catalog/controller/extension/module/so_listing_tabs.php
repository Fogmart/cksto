...
class ControllerExtensionModuleSolistingtabs extends Controller {
	public function index($setting) {
		...
		if($is_ajax && isset($_POST['is_ajax_listing_tabs']) && $_POST['is_ajax_listing_tabs']){
			...
			ob_start();
			// Check cache lite
			$url_default_tdd = $this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/default.tpl';
			$url_items_tdd = $this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/default_items.tpl';
			$url_tem = 'default/template/extension/module/so_listing_tabs/default_items.tpl';
			$url_default_ct = $this->config->get('config_template') . '/template/extension/module/so_listing_tabs/default.tpl';
			$url_items_ct = $this->config->get('config_template') . '/template/extension/module/so_listing_tabs/default_items.tpl';
			if ($use_cache){
			...
			if ($use_cache){
				...
				if (!$_data) {
					$_data = $this->load->view('extension/module/so_listing_tabs/default.tpl', $data);
				...
				if(file_exists($folder_cache))
				$Cache_Lite->_cleanDir($folder_cache);
				return $this->load->view('extension/module/so_listing_tabs/default.tpl', $data);
			}
		}
	}
