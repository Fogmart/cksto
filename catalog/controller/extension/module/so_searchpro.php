...
class ControllerExtensionModuleSosearchpro extends Controller {
	...
	public function index( $setting ) { 
		...
		/*End Entry */
		$this->data['theme_typeheader']  = $this->soconfig->get_settings('typeheader');
		/*Data */
		...
		//=== Theme Custom Code====
		$this->data['type_layout'] = isset($setting['type_layout']) ? $setting['type_layout']:'1';
		...
		if ($use_cache){
			...
			if (!$_data) {
				...
				if(version_compare(VERSION, '2.1.0.2', '>')) {
					$_data = $this->getLayoutMod('so_searchpro',$this->data,$this->data['type_layout']);
				}else{
