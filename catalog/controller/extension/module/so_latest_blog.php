...
class ControllerExtensionModuleSolatestblog extends Controller {
	...
	if(!$this->checkDatabase()) {	
		...
		$data['type_layout'] 			= isset($setting['type_layout'])?$setting['type_layout']: 1;
		...
		if ($use_cache){
			...
			if (!$_data) {
				...
				if(version_compare(VERSION, '2.1.0.2', '>')) {
					//$_data = $this->load->view('extension/module/so_latest_blog/default.tpl', $data);
			...
			if(version_compare(VERSION, '2.1.0.2', '>')) {
				...
				$_data = $this->getLayoutMod('so_latest_blog',$data,$data['type_layout']);
				return  $_data;
				//return $this->load->view('extension/module/so_latest_blog/default.tpl', $data);
			}else{
				$tem_url = $this->config->get('config_template') . '/template/extension/module/so_latest_blog/default.tpl';
				$template_file = DIR_TEMPLATE . $tem_url ? DIR_TEMPLATE . $tem_url : '';
				if (file_exists($template_file)) {
