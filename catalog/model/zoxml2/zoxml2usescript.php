    ...
    if (!$query->row) {
      // ТОВАРА НЕТ В СВЯЗКАХ! ИЩЕМ ЕГО РОДИТЕЛЯ
       ...
       if (!$query->row) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "hpmodel_links SET image = (SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'), product_id = '" . (int)$product_id . "', parent_id = '" . (int)$product_id . "'");
        }
      else {
